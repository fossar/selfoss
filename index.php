...
$router->get('/', function() use ($container): void {
    // json
    $container->make(controllers\Index::class)->home();
});
$router->get('/api/about', function() use ($container): void {
    // json
    $container->make(controllers\About::class)->about();
});
$router->post('/api/private/hash-password', function() use ($container): void {
    // json
    $container->make(controllers\Helpers\HashPassword::class)->hash();
});
$router->get('/login', function() use ($container): void {
    // json, deprecated
    $container->make(controllers\Authentication::class)->login();
});
...
$router->get('/logout', function() use ($container): void {
    // json, deprecated
    $container->make(controllers\Authentication::class)->logout();
});
...
$router->get('/update', function() use ($container): void {
    // text
    $container->make(controllers\Sources\Update::class)->updateAll();
});

// only for loggedin users or on public mode
$router->get('/rss', function() use ($container): void {
    // rss
    $container->make(controllers\Rss::class)->rss();
});
...
$router->post('/source/([0-9]+)/update', function(string $id) use ($container): void {
    // json
    $container->make(controllers\Sources\Update::class)->update($id);
});
$router->get('/sources/spouts', function() use ($container): void {
    // json
    $container->make(controllers\Sources::class)->spouts();
});

$router->post('/tags/color', function() use ($container): void {
    // json
    $container->make(controllers\Tags::class)->color();
});

$router->post('/opml', function() use ($container): void {
    // json
    $container->make(controllers\Opml\Import::class)->add();
});
$router->get('/opmlexport', function() use ($container): void {
    // xml
    $container->make(controllers\Opml\Export::class)->export();
});
