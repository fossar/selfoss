	...
	static function autoload($class) {
		...
		foreach (array(str_replace('\\','_',$class),$class) as $style)
			...
			foreach (self::split(self::$vars['PLUGINS'].';'.
				...
				if (!$abs)
					continue;
				$raw=self::fixslashes($style);
				if (is_int(strpos($raw,'/'))) {
					$ok=FALSE;
					// Case-insensitive check for folders
					foreach (explode('/',
						self::fixslashes(dirname($raw))) as $dir)
						foreach (glob($abs.'/*',GLOB_ONLYDIR) as $found) {
							$found=self::fixslashes($found);
							if (strtolower($abs.'/'.$dir)==
								strtolower($found)) {
								$abs=self::fixslashes($found);
								$ok=TRUE;
							}
						}
					if (!$ok)
				...
				$glob=glob($abs.'/*',GLOB_NOSORT);
				if ($glob) {
					// Case-insensitive check for file presence
					$found=preg_grep('/^'.preg_quote($abs.'/'.$raw,'/').
						'(?:\.class)?\.php/i',
						array_map('self::fixslashes',$glob));
					if ($found && ($file=current($found)) &&
						!in_array($file,$list)) {
						$instance=new F3instance;
