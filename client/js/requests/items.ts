 ...
 */
export function getItems(filter: QueryFilter, abortController?: AbortController): Promise<GetItemsResponse> {
    return ajax.get('', {
 ...
 */
export function sync(updatedStatuses: Array<StatusUpdate>, syncParams: SyncParams): { controller: AbortController, promise: Promise<GetItemsResponse> } {
    const params = {
        ...syncParams,
        updatedStatuses: syncParams.updatedStatuses ? syncParams.updatedStatuses.map((status: StatusUpdate) => {
            return {
                ...status,
